...
export default function TaskDataSelectionScreen() {
  ...
  console.log('params?.returnTab21222', params?.returnTo);
    ...
    try {
      ...
      if (res && res.success && Array.isArray(res.data)) {
        const mapped = res.data.map((p: any) => ({
          id: String(p.id),
          name: `${p.project_id} - ${p.project_name}`,
        }));
        setItems(mapped);
      } else {
        setItems([]);
    ...
    try {
      ...
      if (res && res.success && Array.isArray(res.data)) {
        const mapped = res.data.map((it: any) => ({
          id: String(it.id),
          name: `${it.title} (${it.inspection_type}) - ${
            it.project?.project_id || ''
          }`,
        }));
        setItems(mapped);
      } else {
    ...
    try {
      ...
      if (res && res.success && res.data) {
        ...
        if (dataKey === 'designation') {
          setItems(
            (res.data.designations || []).map((d: any) => ({
              id: String(d.id),
              name: d.name,
            })),
          );
  ...
  // Reset search query when dataKey changes or component unmounts
  React.useEffect(() => {
    setSearchQuery(''); // Reset search query when dataKey changes

    return () => {
      setSearchQuery(''); // Clear search query when component unmounts
    };
  }, [dataKey]); // Add dataKey to dependency array
  ...
  const handleBack = () => {
    // Get the current route parameters from the URL
    ...
    // If there's a returnTo parameter, use it to navigate back
    if (returnTo) {
      // Prepare the navigation parameters
      const navParams: any = {};

      // Preserve the projectId if it exists
      if (params.projectId) {
        navParams.projectId = params.projectId;
      }

      // Preserve any other relevant parameters
      if (params.returnTab) {
        navParams.returnTab = params.returnTab;
      }

      // Navigate back to the specified screen
      router.push({
        pathname: `/(drawer)/${returnTo}`,
        params: navParams,
      });
    } else {
